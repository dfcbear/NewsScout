...
TelegramGateway, WhatsAppGateway, and SignalGateway.
"""
...
# ============================================================================
# 3. WhatsApp Gateway Tests
# ============================================================================

class TestWhatsAppGateway:
    def test_normalize_chat_id(self):
        assert WhatsAppGateway.normalize_chat_id("+49 170 1234567") == "491701234567@c.us"
        assert WhatsAppGateway.normalize_chat_id("491701234567@c.us") == "491701234567@c.us"
        assert WhatsAppGateway.normalize_chat_id("123456-7890@g.us") == "123456-7890@g.us"

    def test_format_whatsapp_card(self, sample_card):
        text = WhatsAppGateway.format_whatsapp_card(sample_card)
        ...
        assert "[#42]" in text
        # Bare url
        assert "🔗 https://github.com/vllm-project/vllm" in text
        # No html tags
    ...
    def test_is_enabled_logic(self):
        s_disabled = Settings(whatsapp_enabled=False)
        gw = WhatsAppGateway(settings=s_disabled)
        assert gw.is_enabled() is False

        s_enabled = Settings(
            whatsapp_enabled=True,
            whatsapp_bridge_url="http://localhost:3000",
            whatsapp_recipients=["491701234567@c.us"],
        )
        gw_enabled = WhatsAppGateway(settings=s_enabled)
        assert gw_enabled.is_enabled() is True
        assert gw_enabled.channel_name == "whatsapp"
    ...
    async def test_send_card_success(self, sample_card):
        def handler(request: httpx.Request) -> httpx.Response:
            ...
            assert body["chatId"] == "491701234567@c.us"
            assert "*vLLM: High-Throughput Serving*" in body["text"]
            return httpx.Response(200, json={"id": "wamid.HBgMNDkx..."})
        ...
        gw = WhatsAppGateway(settings=settings, client=client)

        receipt = await gw.send_card(sample_card)
        assert receipt.success is True
        assert receipt.channel == "whatsapp"
        assert receipt.message_id == "wamid.HBgMNDkx..."
        assert receipt.recipient == "491701234567@c.us"
        await client.aclose()

    @pytest.mark.asyncio
    async def test_send_card_no_recipient_error(self, sample_card):
        settings = Settings(
            whatsapp_enabled=True,
            whatsapp_bridge_url="http://mock-waha:3000",
            whatsapp_recipients=[],
        ...
        gw = WhatsAppGateway(settings=settings)
        receipt = await gw.send_card(sample_card)
        assert receipt.success is False
        assert "No WhatsApp recipient" in receipt.error

    @pytest.mark.asyncio
    async def test_send_card_connection_failure_shielded(self, sample_card):
        def handler(request: httpx.Request) -> httpx.Response:
    ...
    async def test_send_audio_success(self, tmp_path):
        ...
        audio_file.write_bytes(b"FAKE_AUDIO_DATA_FOR_WHATSAPP")

        def handler(request: httpx.Request) -> httpx.Response:
            assert request.url.path == "/api/sendFile"
            body = json.loads(request.read())
            assert body["chatId"] == "491701234567@c.us"
            assert body["file"]["mimetype"] == "audio/mpeg"
            assert base64.b64decode(body["file"]["data"]) == b"FAKE_AUDIO_DATA_FOR_WHATSAPP"
            return httpx.Response(200, json={"id": "wamid.audio123"})
        ...
        receipt = await gw.send_audio(str(audio_file), caption="Morning Briefing")
        assert receipt.success is True
        assert receipt.message_id == "wamid.audio123"
        await client.aclose()
    ...
    async def test_send_audio_missing_file(self, tmp_path):
        ...
        gw = WhatsAppGateway(settings=settings)
        receipt = await gw.send_audio(str(tmp_path / "missing.mp3"))
        assert receipt.success is False
        assert "Audio file not found" in receipt.error
    ...
    async def test_send_text_success(self):
        def handler(request: httpx.Request) -> httpx.Response:
            assert request.url.path == "/api/sendText"
            return httpx.Response(200, json={"id": "wamid.text123"})
        ...
        receipt = await gw.send_text("Hello WhatsApp")
        assert receipt.success is True
        assert receipt.message_id == "wamid.text123"
        await client.aclose()
    ...
    @pytest.mark.asyncio
    async def test_pairing_status_qr_image(self):
        raw_png = b"\x89PNG\r\n\x1a\n\x00\x00mockqr"

        def handler(request: httpx.Request) -> httpx.Response:
            if request.url.path == "/api/sessions/default":
                return httpx.Response(200, json={"status": "SCAN_QR_CODE"})
            elif request.url.path == "/api/default/auth/qr":
